...
const fs = require('fs')
var xpPath = "/Users/Axel/Desktop/nsc/nsc/storage.json"
var xpRead = fs.readFileSync(xpPath);
var xpFile = JSON.parse(xpRead);
var log = "/Users/Axel/Desktop/nsc/nsc/log.txt"
...
client.on('messageCreate', message =>{
    ...
    if(message.channel.name === 'no-sense-chat' && !message.author.bot){
        ...
        client.channels.cache.get('993563189956595792').send(":warning: Doublon potentiel sur " + message.content + " " + "<@&" + 988463484025053184n + ">")
        }
    
        });
    
        var d = new Date();
        client.channels.cache.get('993563189956595792').send(":white_check_mark: " + message.content + ' envoyé par ' + message.author.username + ' le ' + d.toLocaleString());
...
client.on("messageDelete", (message) => {
    if(message.channel.name === 'no-sense-chat' && !message.author.bot) {
    client.channels.cache.get('993563189956595792').send(":question: Soumission suivante supprimée : " + message.content)
    }
    });

client.login(process.env.EXPTOKEN);